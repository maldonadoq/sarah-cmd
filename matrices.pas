...
function StrToMatriz(tx: string): TMatriz;
...
begin
  ...
  x:= NChar(tx,':')+1;
  y:= NCharToFind(tx,',',':')+1;
  Result:= TMatriz.Create(x,y);
  sb:= ':';
  for i:=0 to x-1 do begin
    if i=x-1 then sb:= ']';
    for j:=0 to y-1 do begin
      idx:= idxt+1;
      if j=y-1 then idxt:= IdxStr(tx,sb,idx)
      else idxt:= IdxStr(tx,',',idx);
      sn:= SubString(tx,idx,idxt-1);
      Result.M[i,j]:= StrToFloat(sn);
    end;
  end;
end;

function TMatriz.ToStr(): string;
var
  i,j: integer;
begin
  Result:= '';
  for i:=0 to x-1 do begin
    Result:= Result+'  ';
    for j:=0 to y-1 do
      Result:= Result+' '+FloatToStr(M[i,j]);
    if i<>(x-1) then
      Result:= Result+#13#10;
  end;
end;
